...
def download_course(
    url, folder, videos_download, exercise_download, datasets_download, number=None
):
    ...
    sys.stdout.write(f"{bcolors.BKGREEN} {title}  {bcolors.BKENDC}\n")

    download_slides(course_id, os.path.join(folder, title))

    if exercise_download:
        download_exercises(course_id, os.path.join(folder, title))
    if datasets_download:
        download_datasets(url, os.path.join(folder, title))
    if videos_download:
        download_videos(course_id, os.path.join(folder, title))
...
def download_slides(course_id, folder):
    ...
    slide_links = set(re.findall(r"(https?://s3.[/|\w|:|.|-]+[^/])&", page.text))
    ...
    for link in slide_links:
        helper.download_file(con, link, os.path.join(folder, link.split("/")[-1]))


def download_exercises(course_id, folder):
    chapters = get_course_chapters(course_id)
    sys.stdout.write(f"{bcolors.BOLD}Downloading exercises...{bcolors.ENDC}\n")
...
def download_videos(course_id, folder):
    ...
    for chapter in chapters["user_chapters"]:
        ...
        video_type = 1
        if len(video_ids) == 0:
            video_ids = set(re.findall(r"(//videos.[/|\w|:|.|-]+[^/])&", page.text))
            video_type = 2
        if len(video_ids) == 0:
        ...
        if display_text:
            sys.stdout.write(f"{bcolors.BOLD}Downloading videos...{bcolors.ENDC}\n")
            display_text = False
        ...
        for video_id in video_ids:
            ...
            video_url = json.loads(soup.find("input", {"id": "videoData"})["value"])
            ...
            if link.endswith("mp4") and not link.startswith("http"):
                ...
                if video_type == 1:
                    ...
                    if link_name is not None:
                        ...
                        name = (
                            video_url["audio_link"].split("/")[-1].split(".")[0]
                            + ".mp4"
                        )
                elif video_type == 2:
...
def download_datasets(link, folder):
    ...
    dataset = soup.findAll(
        "a", {"href": re.compile("^https"), "class": re.compile("^link-borderless")}
    )
    if len(dataset) == 0:
        sys.stdout.write(f"{bcolors.FAIL}No dataset found!{bcolors.ENDC}\n")
        return
